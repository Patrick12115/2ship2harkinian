...
std::string GetItemCounts(TrackerItemType itemType, u32 itemId) {
    ...
    switch (itemType) {
        case TRACKER_ITEM_RANDO: {
            switch (itemId) {
                case RI_TRIFORCE_PIECE: {
                    if ((IS_RANDO || IS_ARCHI) && RANDO_SAVE_OPTIONS[RO_SHUFFLE_TRIFORCE_PIECES]) {
                ...
                case RI_GS_TOKEN_OCEAN:
                case RI_GS_TOKEN_SWAMP: {
                    auto max = (IS_RANDO || IS_ARCHI) ? RANDO_SAVE_OPTIONS[RO_SKULLTULA_TOKENS_REQUIRED]
                ...
                case RI_WOODFALL_STRAY_FAIRY: {
                    auto max = (IS_RANDO || IS_ARCHI) ? RANDO_SAVE_OPTIONS[RO_STRAY_FAIRIES_REQUIRED]
