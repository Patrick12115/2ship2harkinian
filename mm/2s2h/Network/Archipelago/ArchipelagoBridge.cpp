...
void CacheCheckedLocations(const std::set<int64_t>& checkedLocations) {
    ...
    if (sSaveReadyForArchiInit && gSaveContext.save.shipSaveInfo.saveType == SAVETYPE_ARCHI) {
...
static bool IsArchiSaveValidNoInit() {
    ...
    const auto& archi = gSaveContext.save.shipSaveInfo.archipelago;
...
void ApplySlotOptions(const nlohmann::json& slotData) {
    ...
    if (sSaveReadyForArchiInit && gSaveContext.save.shipSaveInfo.saveType == SAVETYPE_ARCHI &&
        IsArchiSaveValidNoInit()) {

        auto& archi = gSaveContext.save.shipSaveInfo.archipelago;
...
void EnqueueItem(uint64_t itemId, int fromPlayer, int64_t index, uint32_t flags, const std::string& itemName) {
    ...
    if (index >= 0) {
        if (!sSaveReadyForArchiInit) {
            ...
            uint32_t currentReceivedCount = gSaveContext.save.shipSaveInfo.archipelago.receivedItemCount;
...
static bool GetCheckedBit(uint64_t locationId) {
    ...
    return (gSaveContext.save.shipSaveInfo.archipelago.checkedLocations[byteIndex] & bitMask) != 0;
}
...
static void SetCheckedBit(uint64_t locationId) {
    ...
    const uint64_t byteIndex = locationId / 8;
    const uint8_t bitMask = (uint8_t)(1u << (locationId % 8));
    uint8_t& b = gSaveContext.save.shipSaveInfo.archipelago.checkedLocations[byteIndex];
    if ((b & bitMask) == 0) {
        b |= bitMask;
        gSaveContext.save.shipSaveInfo.archipelago.checkedLocationCount++;
    }
}
...
static void PersistProcessedItemIndexIfNeeded(const PendingItem& item) {
    ...
    if (!IsArchiSaveValidNoInit()) {
        SPDLOG_WARN("[AP][Bridge] Persist skipped: invalid archi save (magic={:#x} version={}) item index={}",
                    gSaveContext.save.shipSaveInfo.archipelago.magic,
                    gSaveContext.save.shipSaveInfo.archipelago.version, item.index);
        return;
    }
