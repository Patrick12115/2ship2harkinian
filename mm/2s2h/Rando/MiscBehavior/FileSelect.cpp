...
void Rando::MiscBehavior::InitFileSelect() {
    ...
    GameInteractor::Instance->RegisterGameHook<GameInteractor::OnFileSelectSaveLoad>(
        [](s16 fileNum, bool isOwlSave, SaveContext* saveContext) {
            ...
            isArchi[fileIndex] = saveContext->save.shipSaveInfo.saveType == SAVETYPE_ARCHI;
            ...
            if (isArchi[fileIndex]) {
                const char* savedSlotName = saveContext->save.shipSaveInfo.archipelago.slotName;
                const char* currentSlotName = CVarGetString("gArchipelago.Slot", "");

                // If save has no slot name but we're connected, save the current slot name
                if (savedSlotName[0] == '\0' && currentSlotName[0] != '\0') {
                    strncpy(saveContext->save.shipSaveInfo.archipelago.slotName, currentSlotName, 31);
                    saveContext->save.shipSaveInfo.archipelago.slotName[31] = '\0';
                    // Note: This will be persisted when the save is next saved
                }

                // Store in our tracking array
                strncpy(fileSlotNames[fileIndex], saveContext->save.shipSaveInfo.archipelago.slotName, 31);
