...
void Rando::MiscBehavior::OnFileLoad() {
    ...
    COND_HOOK(OnFlagSet, (IS_RANDO || IS_ARCHI), Rando::MiscBehavior::OnFlagSet);
    COND_HOOK(OnSceneFlagSet, (IS_RANDO || IS_ARCHI), Rando::MiscBehavior::OnSceneFlagSet);
    COND_HOOK(BeforeEndOfCycleSave, (IS_RANDO || IS_ARCHI), Rando::MiscBehavior::BeforeEndOfCycleSave);
    COND_HOOK(AfterEndOfCycleSave, (IS_RANDO || IS_ARCHI), Rando::MiscBehavior::AfterEndOfCycleSave);
    COND_HOOK(OnSceneInit, (IS_RANDO || IS_ARCHI), Rando::MiscBehavior::OnSceneInit);
    COND_ID_HOOK(OnActorUpdate, ACTOR_PLAYER, (IS_RANDO || IS_ARCHI),
                 [](Actor* actor) { Rando::MiscBehavior::CheckQueue(); });

    // This overrides the ocarina condition for Termina Field
    COND_VB_SHOULD(VB_TERMINA_FIELD_BE_EMPTY, (IS_RANDO || IS_ARCHI), { *should = false; });

    // Override faster first-cycle time speed if you don't have the Ocarina
    COND_VB_SHOULD(VB_FASTER_FIRST_CYCLE, (IS_RANDO || IS_ARCHI), { *should = false; });
    ...
    COND_VB_SHOULD(VB_CHECK_FOR_ROOM_KEY, (IS_RANDO || IS_ARCHI),
                   { *should = Flags_GetRandoInf(RANDO_INF_OBTAINED_ROOM_KEY); });
    ...
    COND_VB_SHOULD(VB_ITEM_GIVE_SWORD_SET_FORM_EQUIP, (IS_RANDO || IS_ARCHI), {
        *should = false;
    ...
    COND_VB_SHOULD(VB_GRANT_MAGIC_UPON_REQUEST, (IS_RANDO || IS_ARCHI), {
    ...
    COND_VB_SHOULD(VB_MEET_MOON_REQUIREMENTS, (IS_RANDO || IS_ARCHI),
                   { *should = Rando::Logic::MeetsMoonRequirements(); });

    // Fix issue where bombchus/bombs can't be used in Honey and Darling if the player has no sword equipped and no bow.
    COND_VB_SHOULD(VB_CLEAR_B_BUTTON_FOR_NO_BOW, (IS_RANDO || IS_ARCHI), {
